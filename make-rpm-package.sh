...
RELEASE="0.5" # release. can be override using --release
TARGET=$DIR/rpm_dist  #output directory for RPMS and TARGZ
RPMTARGET=$TARGET/rpm
TARTARGET=$TARGET/tar
MODE="build"
PROJECT_NAME="watcherd"
SOURCEDIR="./src"
SPECFILE="rpm/watcherd.spec"
...
build() {
    ...
    # build rpm
    rpmbuild -ba ~/rpmbuild/SPECS/$PROJECT_NAME.spec --define="project_release $RELEASE" --define="project_version $VERSION" --define="project_name $PROJECT_NAME"
